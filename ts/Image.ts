    ...
    static getWithItemTypeName(name: string, tile_size: number, seed: number = 0, isTaken: boolean = false, takeable: boolean = true): Image {
        ...
        if (name === "armorer's bench") {
            ...
            const highlandFilters: Readonly<Record<string, string>> = {
                ...
                "highland mountain ground":
                    "brightness(.38) saturate(.35) contrast(1.16)",
                "highland castle floor":
                    "brightness(.6) saturate(.4) sepia(.12)",
            };
            ...
            rotate = rotationSeed % 4 * 90
                + (Image.visualSeed(seed, name, 7) % 25) - 12;
            dimension = 1.25 + (dimensionSeed % 156) / 100;
            const terrainFilters: Readonly<Record<string, string>> = {
                "dungeon fungal floor":
                    "brightness(.42) sepia(.45) hue-rotate(230deg) saturate(.7)",
                "dungeon bone floor":
                    "grayscale(.7) sepia(.25) brightness(.58)",
                "dungeon bazaar floor":
                    "sepia(.5) brightness(.58) saturate(.65)",
                "dungeon forge floor":
                    "sepia(.75) hue-rotate(330deg) brightness(.34) saturate(.75)",
                "dungeon chapel floor":
                    "grayscale(.75) sepia(.2) brightness(.32)",
                "dungeon web floor":
                    "grayscale(.9) brightness(.54) contrast(1.15)",
                "dungeon moss floor":
                    "sepia(.65) hue-rotate(55deg) brightness(.45) saturate(.8)",
                "dungeon crystal floor":
                    "grayscale(.55) hue-rotate(150deg) brightness(.52) saturate(.5)",
            };
