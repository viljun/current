...
export class Effects {
    ...
    private static readonly COMBAT_ITEMS = new Set([
    ...
    static showAreaExplosion(map: HTMLElement, seed: number): void {
        try {
            ...
            host.querySelector(".area-explosion-effect")?.remove();
            map.classList.remove("area-explosion-map-burst");

            const overlay = document.createElement("div");
            overlay.className = "area-explosion-effect";
            overlay.setAttribute("aria-hidden", "true");
            const direction = Effects.areaExplosionSeed(
                seed,
                0,
                0x1b873593,
            ) % 2 === 0 ? -1 : 1;
            const turn = 1.4 + Effects.areaExplosionSeed(
                seed,
                0,
                0x85ebca6b,
            ) % 111 / 100;
    ...
    static showFightOutcome(
        ...
        try {
            ...
            const duration = reducedMotion
                ? defeated ? 3_000 : 2_000
                : defeated ? 5_500 : 4_000;
    ...
    static dealFightCards(
        ...
        try {
            cardElements.forEach((card, index) => {
                const x = Effects.fightCardVariation(
                    owner,
                    round,
                    index,
                    "landing-x",
                ...
                const y = Effects.fightCardVariation(
                    owner,
                    round,
                    index,
                    "landing-y",
                ...
                const angle = Effects.fightCardVariation(
                    owner,
                    round,
                    index,
                    "landing-angle",
            ...
            window.requestAnimationFrame(() => {
                const table = cardElements[0]?.closest<HTMLElement>(".fight-table");
                const targets = cardElements.map(card =>
                    card.getBoundingClientRect()
                );
                const handCenterX = targets.reduce(
                    (total, target) => total + target.left + target.width / 2,
                    0,
                ) / targets.length;
                const handCenterY = targets.reduce(
                    (total, target) => total + target.top + target.height / 2,
                    0,
                ) / targets.length;
                const deckWidth = cardElements[0]?.offsetWidth
                    ?? targets[0]?.width ?? 0;
                const deckHeight = cardElements[0]?.offsetHeight
                    ?? targets[0]?.height ?? 0;
                const chair = table
                    ?.closest<HTMLElement>(".fight-board")
                    ?.querySelector<HTMLElement>(".fight-chair--" + owner);
                const chairBounds = chair?.getBoundingClientRect();
                const edgePadding = 12;
                const sideGap = Math.max(
                    8,
                    Math.min(20, window.innerWidth * 0.02),
                );
                ...
                const preferredCenterX = chairBounds
                    ...
                    : handCenterX;
                const deckCenterX = Math.max(
                    minimumCenterX,
                    Math.min(maximumCenterX, preferredCenterX),
                );
                const preferredCenterY = chairBounds
                    ? chairBounds.top + chairBounds.height / 2
                    : handCenterY;
                const deckCenterY = Math.max(
                    edgePadding + deckHeight / 2,
                    Math.min(
                        window.innerHeight - edgePadding - deckHeight / 2,
                        preferredCenterY,
                    ),
                );
                ...
                const host = table?.closest<HTMLElement>(".fight-overlay")
                    ?? document.body;
                host.append(deck);
                const deckAngle = Effects.fightCardVariation(
                    owner,
                    round,
                    0,
                    "deck-angle",
                ...
                cardElements.forEach((card, index) => {
                    const target = targets[index] ?? card.getBoundingClientRect();
                    ...
                    window.setTimeout(
                        finish,
                        flyInDuration + dealDuration + 140
                            + index * dealDelay,
                    );
                });
    ...
    private static showChanges(
        ...
        const totalAnimationCount = visibleChanges.reduce(
            (total, [, quantity]) => total + Math.min(
                quantity,
    ...
    private static floatItem(
        ...
        originSpread: { x: number, y: number },
        itemIndex: number,
        typeIndex: number,
        rare: boolean,
        actionSeed: number,
        ...
        const startOffset = Effects.itemFlightStartOffset(
            actionSeed,
            itemName,
            itemIndex,
            typeIndex,
    ...
    static itemFlightStartOffset(
        ...
        const angleSeed = Effects.flightStreamSeed(
            actionSeed,
            itemName,
            itemIndex,
            typeIndex,
        ...
        const radiusSeed = Effects.flightStreamSeed(
            actionSeed,
            itemName,
            itemIndex,
            typeIndex,
        ...
        const angle = angleSeed / 0x1_0000_0000 * Math.PI * 2;
        const radius = Math.sqrt(radiusSeed / 0x1_0000_0000);
    ...
    private static monsterReadableAngle(cardId: string): number {
        const directionSeed = Effects.stringSeed(
            "monster-reveal-direction:" + cardId,
        );
        const magnitudeSeed = Effects.stringSeed(
            "monster-reveal-magnitude:" + cardId,
        );
    ...
    private static async flipMonsterCard(
        ...
        const landingAngle = Number(cardElement.dataset.dealAngle ?? "0");
