...
export class ItemExplanation {
    ...
    private static readonly ITEM_FIELD_NOTES: Readonly<
    ...
    private static fieldNoteFor(itemName: string, key: string): string {
        ...
        if (exactNotes !== undefined) {
            ...
            const recipe = ItemExplanation.pick(
                craftingUses,
                key,
                "field-note-recipe",
            );
        ...
        const battleSpell = BattleSpell.get(itemName);
        if (battleSpell !== null) {
            const effectPhrases: Readonly<Record<string, string>> = {
        ...
        if (itemName.startsWith(buyingPrefix)
            ...
            const merchandise = itemName.slice(
        ...
        if (effects !== null && effects.damage > 0) {
            return ItemExplanation.weaponFieldNote(
                itemName,
                effects.damage,
                key,
            );
        ...
        const craftingUses = ItemExplanation.craftingUses(itemName);
        if (craftingUses.length > 0) {
            const firstRecipe = craftingUses[0] ?? "future equipment";
            return ItemExplanation.pick([
    ...
    private static monsterFieldNote(itemName: string, key: string): string {
