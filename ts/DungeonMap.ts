...
export class DungeonMap {
    private static readonly FEATURE_CHUNK_SIZE = 24;
    private static readonly FEATURE_KINDS: readonly DungeonFeatureKind[] = [
        "moonwell",
        "sand vault",
        "gloamcap grove",
        "boneyard",
    ...
    static forViewport(
        ...
        const origin = new GameCoordinates(
            center.latitude + (rows + 1) / 2 + extraSize,
            center.longitude - (cols + 1) / 2 - extraSize,
        );
        ...
        const features = DungeonMap.nearbyFeatures(coordinates)
            ...
            .sort((first, second) => {
                const distanceDifference =
                    DungeonMap.featureDistance(first, coordinates)
    ...
    static featureTitleAt(coordinates: Coordinates): string|null {
        ...
        const titles: Readonly<Record<DungeonFeatureKind, string>> = {
    ...
    static terrainAt(coordinates: Coordinates): string {
        ...
        if (feature.kind === "moonwell") {
            const lakeDistance = Math.pow(
                local.x / Math.max(1, feature.radiusX - 2),
                2,
            ) + Math.pow(
                local.y / Math.max(1, feature.radiusY - 2),
                2,
            );
        ...
        const terrain: Readonly<Record<
            Exclude<DungeonFeatureKind, "moonwell">,
            string
        >> = {
    ...
    static itemAt(coordinates: Coordinates): ItemType|null {
        const baseItem = ItemType.getWithSeed(
            coordinates.getSeed(),
            DUNGEON_AREA,
        );
        if (baseItem?.name === "stairs up") {
            return baseItem;
        }
        const feature = DungeonMap.featureAt(coordinates);
        if (feature === null) {
    ...
    private carveFeatureRooms(dungeonMap: boolean[][]): boolean[][] {
        for (let col = 0; col <= this.width; col++) {
            for (let row = 0; row <= this.height; row++) {
                const coordinates = this.coordinatesAt(col, row);
                if (!DungeonMap.isFeatureFloorAt(coordinates)) {
                ...
                const dungeonRow = dungeonMap[row] ?? [];
    ...
    private static featureItemAt(
        ...
        if (feature.kind === "boneyard") {
            if (center || seed % 19 === 0) {
                return new ItemType(
                    seed % 2 === 0 ? "skeletal guard" : "armored skeleton",
                );
            }
        ...
        if (feature.kind === "spider nursery") {
            if (center || seed % 23 === 0) {
                return new ItemType(
                    seed % 2 === 0 ? "giant spider" : "brood spider",
                );
            }
    ...
    private static nearbyFeatures(
        ...
        const chunkLatitude = Math.floor(
            coordinates.latitude / DungeonMap.FEATURE_CHUNK_SIZE,
        );
        const chunkLongitude = Math.floor(
            coordinates.longitude / DungeonMap.FEATURE_CHUNK_SIZE,
        );
        const features: DungeonFeature[] = [];
        for (let x = -1; x <= 1; x++) {
            for (let y = -1; y <= 1; y++) {
                const feature = DungeonMap.featureForChunk(
                    chunkLatitude + x,
                    chunkLongitude + y,
                );
                if (feature !== null) {
    ...
    private static featureForChunk(
        chunkLatitude: number,
        chunkLongitude: number,
    ): DungeonFeature|null {
        const presenceSeed = DungeonMap.hash(
            chunkLatitude,
            chunkLongitude,
            0x243f6a88,
        );
        ...
        const kindSeed = DungeonMap.hash(
            chunkLatitude,
            chunkLongitude,
            0x85a308d3,
        );
        const kind = DungeonMap.FEATURE_KINDS[
            kindSeed % DungeonMap.FEATURE_KINDS.length
        ];
        if (kind === undefined) {
            return null;
        }
        const centerXSeed = DungeonMap.hash(
            chunkLatitude,
            chunkLongitude,
            0x13198a2e,
        );
        const centerYSeed = DungeonMap.hash(
            chunkLatitude,
            chunkLongitude,
            0x03707344,
        );
        const sizeSeed = DungeonMap.hash(
            chunkLatitude,
            chunkLongitude,
            0xa4093822,
        );
        ...
        return {
            kind,
            centerLatitude:
                chunkLatitude * DungeonMap.FEATURE_CHUNK_SIZE
                + 5 + centerXSeed % 15,
            centerLongitude:
                chunkLongitude * DungeonMap.FEATURE_CHUNK_SIZE
                + 5 + centerYSeed % 15,
            radiusX: baseRadiusX + sizeSeed % 3,
            radiusY: baseRadiusY + (sizeSeed >>> 8) % 3,
            rotated: (sizeSeed & 1) === 1,
            seed: DungeonMap.hash(
                chunkLatitude,
                chunkLongitude,
                0x299f31d0,
            ),
    ...
    private static isInsideRoom(
        ...
        if (feature.kind === "boneyard") {
            ...
            const capDistance = Math.hypot(
                Math.max(0, absoluteX - straightLength),
                absoluteY,
            );
        ...
        if (feature.kind === "black candle chapel") {
            return (
                absoluteX <= 2 && absoluteY <= feature.radiusY
            ) || (
                absoluteY <= 2 && absoluteX <= feature.radiusX
            );
        }
        const irregularity = feature.kind === "gloamcap grove"
            ...
            ? Math.sin(
                (coordinates.latitude + feature.seed % 17) * .83,
            ) * Math.cos(
                (coordinates.longitude - feature.seed % 23) * .71,
            ) * .12
            : 0;
    ...
    private static cellSeed(
        ...
        return DungeonMap.hash(
            coordinates.latitude ^ feature.seed,
            coordinates.longitude + feature.seed,
            salt,
        );
    }
    ...
    private coordinatesAt(col: number, row: number): Coordinates {
        return new GameCoordinates(
            this.coordinates.latitude - row,
            this.coordinates.longitude + col,
        );
    }
