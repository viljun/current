...
export class OriginArtwork {
    static containSubject(
        artwork: HTMLElement,
        frameClassName: string,
    ): void {
        const subject = artwork.querySelector<HTMLElement>(
            ".origin-artwork-subject",
        );
        if (subject === null) {
            return;
        }
        subject.style.setProperty(
            "--origin-artwork-subject-width",
            subject.style.width,
        );
        subject.style.setProperty(
            "--origin-artwork-subject-height",
        ...
        for (let y = -1; y <= 1; y++) {
            for (let x = -1; x <= 1; x++) {
                ...
                if (origin.areaId === 0) {
                    ...
                    if (river !== null) {
                        const visual = SurfaceMap.riverVisualAt(
                            coordinates,
                            river,
                        );
                        ...
                        cell.style.setProperty(
                            ...
                            visual.textureOffsetXInTiles * 54 + inset + "px "
                                + (
                    ...
                    if (road !== null) {
                        roadVisual = OriginArtwork.decorateRoadCell(
                            cell,
                            coordinates,
                            road,
                            54,
                    ...
                    if (river === null && road === null) {
                        ...
                        OriginArtwork.decorateRoadGrass(
                            cell,
                            seed,
                            roadVisual,
                            54,
                    ...
                    const crossing = SurfaceMap.crossingAt(
                        coordinates,
                        road,
                        river,
                    );
                    if (crossing !== null) {
                        cell.classList.add(
                            ...
                            "surface-road-crossing--" + crossing.kind,
                        );
                        if (crossing.bridgeAnchor) {
                            const bridge = GameImage.getWithItemTypeName(
                                "surface road bridge",
                                54,
                                seed,
                            ).element();
                    ...
                    if (milestone) {
                        const milestone = GameImage.getWithItemTypeName(
                    ...
                    cell.append(GameImage.getWithItemTypeName(
                        HighlandMap.terrainAt(coordinates),
                        54,
                        seed,
                    ).element());
                ...
                if (x === 0 && y === 0) {
                    const subject = GameImage.getWithItemTypeName(
                        itemName,
                        54,
                        seed,
                    ).element();
    ...
    private static decorateRoadCell(
        cell: HTMLDivElement,
    ...
    private static decorateRoadGrass(
        ...
        const grass = GameImage.getWithItemTypeName(
            "surface road grass",
            tileSize,
            seed,
        ).element();
