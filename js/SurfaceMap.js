...
export class SurfaceMap {
    ...
    static pathPatchVisualsAt(coordinates, road) {
        if (road.kind !== "path") {
            return [];
        }
        ...
        const stableValue = (index, salt) => SurfaceMap.hash(coordinates.latitude, coordinates.longitude, road.routeId, index, salt);
        const alongSlots = [-.48, -.32, -.16, 0, .16, .32, .48];
        return alongSlots.map((slot, index) => {
            const alongJitter = (stableValue(index, 0x18c2f457) % 19 - 9) / 100;
            const acrossJitter = (stableValue(index, 0xa4d91e63) % 65 - 32) / 100;
