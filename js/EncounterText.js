export class EncounterText {
    static for(itemName, latitude, longitude) {
        const identityKey = [
            itemName,
    ...
    static monsterLabel(itemName, generatedName) {
        const article = /^[aeiou]/i.test(itemName) ? "an" : "a";
        return generatedName + ", " + article + " " + itemName;
    }
    static cat(itemName, identityKey) {
        const name = EncounterText.generatedName(identityKey, "cat-name", EncounterText.CAT_TITLES, EncounterText.CAT_NAME_ROOTS, EncounterText.CAT_NAME_ENDINGS);
        const coat = EncounterText.pick(EncounterText.CAT_COATS, identityKey, "cat-coat");
        const mood = EncounterText.pick(EncounterText.CAT_MOODS, identityKey, "cat-mood");
        const quirk = EncounterText.pick(EncounterText.CAT_QUIRKS, identityKey, "cat-quirk");
        const buying = itemName.startsWith("cat buying ");
        const item = itemName.slice((buying ? "cat buying " : "cat selling ").length);
        const tradeLine = EncounterText.fillItem(EncounterText.pick(buying
            ? EncounterText.CAT_BUYING_LINES
            : EncounterText.CAT_SELLING_LINES, identityKey, "cat-trade-line"), item);
        const claim = EncounterText.fillItem(EncounterText.pick(EncounterText.CAT_TRADE_CLAIMS, identityKey, "cat-trade-claim"), item);
        const catPhrase = EncounterText.withIndefiniteArticle(mood + " " + coat + " cat");
        const coatCatPhrase = EncounterText.withIndefiniteArticle(coat + " cat");
        const templates = [
        ...
        const description = EncounterText.pick(templates, identityKey, "cat-template");
        return {
            name,
    ...
    static creature(itemName, identityKey) {
        const name = EncounterText.generatedName(identityKey, "creature-name", EncounterText.CREATURE_TITLES, EncounterText.CREATURE_NAME_ROOTS, EncounterText.CREATURE_NAME_ENDINGS);
        const adjective = EncounterText.pick(EncounterText.CREATURE_ADJECTIVES, identityKey, "creature-adjective");
        const habit = EncounterText.pick(EncounterText.CREATURE_HABITS, identityKey, "creature-habit");
        const possession = EncounterText.pick(EncounterText.CREATURE_POSSESSIONS, identityKey, "creature-possession");
        const reputation = EncounterText.pick(EncounterText.CREATURE_REPUTATIONS, identityKey, "creature-reputation");
        const creaturePhrase = EncounterText.withIndefiniteArticle(adjective + " " + itemName);
        const templates = [
        ...
        const description = EncounterText.pick(templates, identityKey, "creature-template");
        return {
            name,
            description: EncounterText.fill(description, [
                ["name", name],
    ...
    static magician(itemName, identityKey) {
        const name = EncounterText.pick(EncounterText.MAGICIAN_NAMES, identityKey, "magician-name");
        const note = EncounterText.pick(EncounterText.MAGICIAN_NOTES, identityKey, "magician-note");
        ...
        const description = EncounterText.pick(templates, identityKey, "magician-template");
        return {
            name,
            description: EncounterText.fill(description, [
                ["name", name],
    ...
    static generatedName(identityKey, channel, titles, roots, endings) {
    ...
    static withIndefiniteArticle(phrase) {
    ...
    static pick(values, identityKey, channel) {
        var _a, _b;
        return (_b = (_a = values[EncounterText.hash(identityKey, channel) % values.length]) !== null && _a !== void 0 ? _a : values[0]) !== null && _b !== void 0 ? _b : "";
    }
    static hash(identityKey, channel) {
