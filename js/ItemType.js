...
export class ItemType {
    ...
    static allNames() {
        ...
        return [...names].sort((first, second) => second.length - first.length
    ...
    static getShopOutsideWithSeed(seed) {
        ...
        if ((areaItem === null || areaItem === void 0 ? void 0 : areaItem.name) === "stairs up") {
            return areaItem;
        }
        let name = null;
        if (!(seed % 23)) {
    ...
    static isBonusHighlandEntranceSeed(seed, chanceMultiplier) {
        const modulus = ItemType.HIGHLAND_ENTRANCE_MODULUS;
        const remainder = ((seed % modulus) + modulus) % modulus;
    ...
    static vendorCatPlayerScale(tradeName) {
        const buyingPrefix = "cat buying ";
        const sellingPrefix = "cat selling ";
        ...
        const itemName = tradeName.slice(buying ? buyingPrefix.length : sellingPrefix.length);
        const trade = ItemType.SHOP_TRADES.find(value => value.item === itemName);
        if (trade === undefined) {
            return 1;
        }
        const prices = ItemType.SHOP_TRADES.reduce((values, tradeValue) => values.concat(ItemType.shopPrice(tradeValue.item, tradeValue.quantity, true), ItemType.shopPrice(tradeValue.item, tradeValue.quantity, false)), []);
